...
#pragma mark - POST请求RAW无缓存
+ (void)POST_RAW:(NSString *)URL
                parameters:(id)parameters
                   success:(PPHttpRequestSuccess)success
                   failure:(PPHttpRequestFailed)failure {
    [self POST_RAW:URL parameters:parameters responseCache:nil success:success failure:failure];
}
...
+ (NSURLSessionTask *)POST:(NSString *)URL
             ...
             responseCache:(PPHttpRequestCache)responseCache
                   success:(PPHttpRequestSuccess)success
                   failure:(PPHttpRequestFailed)failure {
    //传入的参数不包含token token在这里在读取处理
    NSString *token = [WJUserdefault valueForKey:@"token"];
    
    if (!WJStrIsEmpty(token)) {
...
+ (void)POST_RAW:(NSString *)URL
    ...
    if (!WJStrIsEmpty([WJUserdefault objectForKey:@"token"])) {
        ...
        [manager.requestSerializer setValue:[WJUserdefault objectForKey:@"token"] forHTTPHeaderField:@"access_token"];
    }
